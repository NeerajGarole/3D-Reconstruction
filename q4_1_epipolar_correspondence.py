...
def epipolarCorrespondence(im1, im2, F, x1, y1):
    ...
    ep0, ep1, ep2 = np.matmul(F, np.transpose(x))
    w_size = 50
    im1_sect = im1[(y1 - int(w_size / 2)): (y1 + int(w_size / 2) + 1),
               (x1 - int(w_size / 2)): (x1 + int(w_size / 2) + 1), :]
    side = np.linspace(-2, 2, w_size + 1)
    x, y = np.meshgrid(side, side)
    sd = 3
    gauss_weight = np.exp(-((x ** 2 + y ** 2) / (2 * (sd ** 2))))
    gauss_weight = np.sum(gauss_weight/np.sqrt(2 * 3.1415 * (sd ** 2)))
    sec1 = gauss_weight * im1_sect
    err_new = np.inf
    for i in range(im2.shape[0] - w_size):
        y2_temp = i + int(w_size / 2)
        x2_temp = -(ep1 * y2_temp + ep2) / ep0
        x2_temp = int(x2_temp)
        im2_sect = im2[(y2_temp - int(w_size / 2)): (y2_temp + int(w_size / 2) + 1),
                   (x2_temp - int(w_size / 2)): (x2_temp + int(w_size / 2) + 1), :]
        d = np.asarray([x1 - x2_temp, y1 - y2_temp])
        d_norm = np.linalg.norm(d)
        sec2 = gauss_weight * im2_sect
        err = np.linalg.norm(sec2 - sec1)
        if err < err_new and d_norm < 50:
            x2 = x2_temp
            y2 = y2_temp
            err_new = err
